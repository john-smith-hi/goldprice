...
@section('title', 'Thêm cài đặt mới')

@section('content')
<div class="card">
    <div class="card-header">
        <h4 class="mb-0">Thêm cài đặt mới</h4>
    </div>
    <div class="card-body">
                ...
                <div class="col-md-6">
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-select @error('type_value') is-invalid @enderror" id="type_value" name="type_value" required>
                            ...
                            <option value="integer" {{ old('type_value') == 'integer' ? 'selected' : '' }}>Integer (Số nguyên)</option>
                            <option value="float" {{ old('type_value') == 'float' ? 'selected' : '' }}>Float (Số thực)</option>
                            <option value="varchar" {{ old('type_value') == 'varchar' ? 'selected' : '' }}>Varchar (Chuỗi)</option>
                        </select>
                        @error('type_value')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
